...
%Answer: The size of third dimension is 3 because the third dimension is 3 since it has RGB.  
...
% Your task: 
%Is there any difference between each run? Why?
%Answer: Yes, because the filter is randomized. 

%What is the size of the output y, and how is this related to x and w?
%Answer: Since Y is the output and W is the filter, the bigger the filter, the smaller will be the output. 
%Similarly,application of filter W in X reduce 2 from the right and 2 from the left of X to give the output Y.         
...
% Your task: 
%How does the size of y_pad differ from previous y? Can you explain why?
%Answer: The size of y_pad is same as the input X, which means that
%applying zero padding saves the number of pixels. 
...
%Answer: Repmat function is needed here to repeat the filter
%in the image in every pixel. 
%            Take a look at the result. 
%What kind of a features does our filter extract?
%Answer: It extracts the edges of the image. 
...
% Your task: 
%Some of the functions in a CNN should be non-linear. Why?
%Answer: Yes, some of the function in a CNN be non-linear. Without a non-linear activation 
%function in the network, a NN, no matter how many layers it had, 
%would behave just like a single-layer perceptron, because summing 
%these layers would give you just another linear function.
...
%Answer: Reduces the number of parameters within the model. 

%What does the 'Stride' parameter do?
%Answer: Stride is a parameter of the neural network's filter that modifies the amount of 
%movement over the image. 
...
%Answer: In ideal case, all negative belongs to 0 classification threshold
%and positive to 1.

%What is the result here compared to the ideal case?
%Answer:  The red curve moves to far left and green curve moves to far
%right. 
...
%Answer: The network does not learn if having too high of a learning rate. 
%    Restore the learning rate and set momentum to 0.
%    How does this differ from the previous with the same learning rate?
%Answer: The erros reduction speed was reduced slowly than the previous one.  
%    What is the benefit of using momentum?
%Answer: Makes the conversion faster, also it prevents in getting stuck in local
%optima. 
